...
if backend_name in ["tensorflow.compat.v1", "tensorflow"]:
    ...
    __all__ = ["jacobian", "hessian", "jacobian_vec"]
else:
    ...
    def __call__(self, i=0, j=None):
        ...
        if i not in self.J:
            if backend_name in ["tensorflow.compat.v1", "tensorflow"]:
                ...
                res = tf.gradients(y, self.xs)[0]
                self.J[i] = res if res is not None else tf.zeros_like(self.xs)
...
def jacobian_vec(output, inputs, use_pfor=True, parallel_iterations=None):
    ...
    """
    flat_inputs = nest.flatten(inputs)  # 일반적인 flatten이 아님에 주의!
    ...
    def loop_fn(i):
        ...
        return gradient_ops.gradients(
            y, flat_inputs, unconnected_gradients=tf.UnconnectedGradients.ZERO
        )
